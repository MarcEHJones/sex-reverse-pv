...
Read in the dataset. This should result in 154 individuals.

```{r}
pog_all <- read_csv(here("raw-data/Pog29.csv")) # n = 154
...
pog <-
  ...
  select(ID, Genotype, Age, BiteForce, BFnolever,
         ...
         LSR_HLroret, LSR_HLroqu, LSR_HWqu, LSR_HWmt, LSR_HDmt, IncTemp, Temp)
```
...
```{r}
write_csv(pog, path = here("data/pogona-data2.csv"))
```
