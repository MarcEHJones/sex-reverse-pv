---
title: "Data wrangling (not needed to reproduce analyses)"
...
>This script requires the raw data. However, not all of this is published, nor does it all completely belong to me, so I have not shared it in this repo. The output of this script contains all the data required to reproduce the analyses in the paper. This script remains in the repo for legacy purposes.
...
pog_all <- read_csv(here("raw-data/Pog33.csv"), na = c("na")) 
...
pog <-
  ...
  mutate(Age = case_when(Age == "A" ~ "Adult", 
                         Age == "J" ~ "Juvenile")) %>%
  # Replace ZZF/ZZM/ZWF with f,m to match text
  mutate(Genotype = case_when(Genotype == "ZZF" ~ "ZZf", 
                              Genotype == "ZZM" ~ "ZZm",
                              Genotype == "ZWF" ~ "ZWf")) %>%
